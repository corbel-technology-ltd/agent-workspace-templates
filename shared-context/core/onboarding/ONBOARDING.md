...
Turn a freshly copied **blank Shared-Context template** into a live, named shared-context store. The
...
```text
<CREATED_DATE> | onboarding | Store initialised from the Shared-Context template; six tokens filled; store blank pending calibration | window: n/a (initialisation)
```
