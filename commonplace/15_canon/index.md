...
> **Status `blank`.** There is no canon yet. Establish it with the owner, then this index lists each
> canon file with its provenance and status.
...
- **Staged before locked.** New direction/brand material enters as `status: staged` (working canon
  under review), not authority. Separate **actual established canon** (genuinely decided and written
  down) from **head-canon and newfound ideas** (aspirational or new) before it is treated as truth.
- **Once settled, it graduates.** Durable pieces lock here (`status: current`); principles that
  belong in memory graduate to long-term atoms. Decisions that shaped the canon trace to the
  decision-log.
- **One concept per file**, frontmatter with `type`, `related:` links for relationships (OKF).
