...
Folder-Agent-Workspace is CORBEL's flavour of a **Filesystem Agent Workspace (FAW)**: an open, file-based pattern
...
Folder-Agent-Workspace is the reference implementation you clone.

Not an autonomous AI CEO. Folder-Agent-Workspace handles what does not need you and prepares what does, then
...
- **Capability-Registry** - the capability registry a fleet fits out from: this template's gate scripts and
  onboarding engine are stocked there, so improvements flow between siblings instead of drifting.

**Taking just this part is fine.** Folder-Agent-Workspace assumes neither sibling: leave
