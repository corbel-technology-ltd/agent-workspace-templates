...
| `core/hooks/session-brief.py` | hook | Inject boot orientation (who/focus/handover/stand-by), open decisions/loops, and offline memory/template maintenance nudges at session start. | session start | `00_meta/staging.md`, `90_runs/` (newest handover), `80_projects/*/loops.md`, `50_registers/decision-queue.md`, `20_memory/_meta/template-check.json` | active |
...
| memory-sleep skill (runtime adapter layer; see `core/RUNTIMES.md`) | skill | The bounded LLM synthesis step between sleep-prep and sleep-apply: proposes strict-JSON claims from staged candidates. | brief nudge, user request | `_meta/sleep-claims.json` | active |
...
| `tools/template-update.py` | tool | Check, classify, and non-clobberingly apply repeated template updates using separate reviewed upstream and accepted local hashes; isolate real upstream deltas as `.template-new` merge candidates. | template-update playbook, manual | managed spine, `00_meta/template-origin.json`, `20_memory/_meta/template-check.json`, user cache | active |
| `tools/update-selftest.py` | tool | Prove repeated update cycles, two-hash acceptance, local-only collisions, legacy stamps, candidate emission and status semantics in disposable repositories. | family gate, manual | temp directories only | active |
...
| template-update skill (runtime adapter layer; see `core/RUNTIMES.md`) | skill | Thin pointer to the neutral safe-template-update playbook. | session nudge, user request | `60_workflows/template-update.md` | active |
