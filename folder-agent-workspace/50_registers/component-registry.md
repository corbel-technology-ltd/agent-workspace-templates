...
| `core/hooks/sleep-apply.py` | tool | Sleep pass stage 2 (deterministic validator + writer): reject unsupported/invented/duplicate claims, write accepted atoms + association edges + world-model snapshot, advance the sleep marker, run the reaper. | memory-sleep skill, manual | depth layers, `subconscious/`, `_meta/sleep-*.{json,md}` | active |
...
| `tools/recall-tiered.py` | tool | Depth-ordered memory search: working -> short-term -> long-term -> subconscious (PRIMING) -> archive (EXPIRED) -> journal (raw truth); stops at first answering depth; --touch feeds retrievals back into ACT-R activation. | manual, any retrieval need | read-only over `20_memory/` (--touch updates touches/retrieval_count) | active |
