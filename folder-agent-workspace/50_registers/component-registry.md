...
| `tools/template-update.py` | tool | Check, classify, and non-clobberingly apply template-managed spine changes using origin hashes and `.template-new` merge candidates. | template-update playbook, manual | managed spine, `00_meta/template-origin.json`, `20_memory/_meta/template-check.json`, user cache | active |
| `tools/update-selftest.py` | tool | End-to-end disposable proof of instantiate, onboard, cached update, pacnew merge, accept, and status semantics. | family check, manual | temp dir only | active |
