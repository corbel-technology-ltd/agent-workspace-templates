...
Turn a freshly copied **blank Capability-Registry template** into a live, named capability registry. Staged
...
|---|---|---|
| `CHANDLERY_NAME` | This registry's name (e.g. `Acme Capability-Registry`) | asked |
| `OWNER` | Who gates installs and packs | asked |
| `capability-registry_slug` | Lowercase id handle | derived from `CHANDLERY_NAME` |
...
**B - gather (read-only).** Ask for `CHANDLERY_NAME` and `OWNER`; derive `capability-registry_slug`
...
```text
<CREATED_DATE> | onboarding | Capability-Registry initialised from the template; four tokens filled | seed stock carried: run `chandler.py list`
```
