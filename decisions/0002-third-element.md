...
name: Decision 0002 - the third element is Capability-Registry, a capability registry + parity ledger
type: decision
status: decided
owner: shared
created: 2026-07-02
tags: [decision, third-element, capability-registry, registry, parity, drift]
---

# Decision 0002 - the third element

**Decided 2026-07-02.** The third member of the family is **Capability-Registry**: a plain-files capability
...
- Folder-Agent-Workspace's constitution says generic improvements "should flow back upstream to the template"
...
vessels, Shared-Context is the star they steer by, and the Capability-Registry is where every vessel fits out with
...
| # | Candidate | Verdict |
|---|---|---|
| 1 | **Capability-Registry** - capability registry + parity ledger + flow-back mechanism | **Chosen.** Strong on all five; grounded in a documented live pain, not speculation. |
| 2 | Cockpit - human command-surface (aggregated briefs, approvals, remote control) | Rejected: violates the family's own value-gate. UI surfaces are explicitly deferred-software in the Folder-Agent-Workspace design spec; a cockpit template would ship the thing the doctrine says to defer. |
...
| 5 | Baton - continuity/relay layer (multi-session handover as a reusable pattern) | Rejected as a member: real but thin. Folder-Agent-Workspace already carries the handover discipline (staging pointer + run notes + re-verify rule). The generalisable remainder is a *capability* - exactly the kind of thing Capability-Registry distributes - not a sibling template. |
| 6 | Gauntlet - agent evaluation layer (scenario banks, scorecards, calibration tests) | Runner-up. Independently useful and ethos-clean, but it amplifies Shared-Context's calibration corner rather than connecting all three, and it needs accumulated instance data to be more than empty scaffolding. Revisit once instances have history to test against. |
...
## Why Capability-Registry wins

- **Load-bearing (criterion 1):** it closes Folder-Agent-Workspace's open flow-back loop (upstreaming gets a
  mechanism: `pack` a capability from an instance, version it, `install` it into siblings) and
  gives Shared-Context's coordination layer the thing it coordinates *about* (the roster of workspaces
  is the fleet; parity offers between workspaces become registry entries instead of prose
  handoffs).
- **Composable and optional (2):** a lone Folder-Agent-Workspace never needs it. Two workspaces want it. It
...
- **Surprising but obvious (5):** the family's own build proves it. The three members vendor the
  same five tools; `capability-registry/registry/` stocks those tools as its seed capabilities and
