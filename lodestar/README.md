...
Lodestar is the shared-context member of the **FAW template family** (Filesystem Agent Workspace):
Markdown + git, deterministic-first, no app, no database, no vendor. Its sibling **Commonplace** is
the workspace an agent runs; Lodestar is the star all of those workspaces steer by.
...
Lodestar is the **shared-context** member of the FAW template family - three templates that stand
alone and click together (the whole story lives in `FAMILY.md` at the family repo root,
`github.com/CORBEL-Technology/Harbour`):

- **Commonplace** - the workspace an agent runs; its onboarding asks for this store's path and its
  session brief loads the shared brain first.
- **Lodestar** (this one) - the store above every workspace.
- **Chandlery** - the capability registry a fleet fits out from; point its `fleet.yml` at this
...
that can read a folder can consume this store, not just Commonplace.
