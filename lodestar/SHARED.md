...
tags: [shared-context, constitution, precedence, link-in, governance, lodestar]
related:
...
Built from **Lodestar**, the shared-context member of the FAW template family (a workspace is to
...
2. **Reach.** The workspace records this store's absolute path in its own constitution (a
   Commonplace workspace fills `SHARED_CONTEXT_PATH` at onboarding; any other layout keeps an
