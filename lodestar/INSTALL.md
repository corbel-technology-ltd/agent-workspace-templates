...
**Lodestar**: a blank, shareable shared-context store - the constitution that sits above every
...
boot rule to paste into that workspace's constitution. A Commonplace workspace needs no paste -
...
Lodestar is the shared-context member of the FAW template family (siblings: **Commonplace**, the
workspace that links in at onboarding; **Chandlery**, the capability registry whose fleet report
cross-checks this store's roster). Composition and take-just-one-part paths: `FAMILY.md` at the
family repo root (`github.com/CORBEL-Technology/Harbour`).
