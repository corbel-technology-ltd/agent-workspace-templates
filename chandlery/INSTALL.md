...
**Chandlery**: a blank, shareable capability registry for a fleet of agent workspaces. This file
...
2. The [`.uninitialised`](.uninitialised) sentinel + the session-start gate route the agent to the
   **onboarding** playbook ([`core/onboarding/ONBOARDING.md`](core/onboarding/ONBOARDING.md)):
   four tokens (chandlery name, owner, slug, date), deterministic fill, first ledger entry.
...
Chandlery is the registry member of the FAW template family (siblings: **Commonplace**, the
workspace the fleet is made of; **Lodestar**, the shared-context store whose roster the fleet
report cross-checks). Composition and take-just-one-part paths: `FAMILY.md` at the family repo
root (`github.com/CORBEL-Technology/Harbour`).
