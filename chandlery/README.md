...
Chandlery is the registry member of the **FAW template family** (Filesystem Agent Workspace).
**Commonplace** is the workspace a fleet is made of; **Lodestar** is the shared brain those
workspaces steer by; the Chandlery is where every vessel fits out with standard, proven gear.
...
- **Alone:** any folder-shaped tooling can be stocked and synced - agent workspaces, dotfiles-like
  setups, anything.
- **With Commonplace:** workspaces track installed capabilities in a `.chandlery.yml` lockfile;
  their component registries stay the per-workspace catalogue while the chandlery holds the
  cross-workspace stock.
- **With Lodestar:** point `fleet.yml`'s `shared_context:` at the store and `fleet` cross-checks
  ...
  chandlery is the mechanism those rules govern.
...
fleet.yml        the workspaces this chandlery outfits
...
Chandlery is the **registry** member of the FAW template family - three templates that stand alone
and click together (the whole story lives in `FAMILY.md` at the family repo root,
`github.com/CORBEL-Technology/Harbour`):

- **Commonplace** - the workspace an agent runs; the fleet is made of these.
- **Lodestar** - the shared-context store the fleet steers by; `fleet.yml` can point at it for
  roster cross-checks.
- **Chandlery** (this one) - where the fleet fits out.
