#!/usr/bin/env python3
"""chandler.py - the deterministic engine of a Chandlery capability registry.
...
Commands:
    ...
    install  NAME --workspace PATH [--yes]      copy payload to targets; write .chandlery.yml
...
def write_lock(ws: Path, installed: dict):
    lp = ws / LOCKFILE
    lp.write_text(
        "# .chandlery.yml - what this workspace has installed from its chandlery.\n"
...
# `shared_context` may name a Lodestar shared-context store root; `fleet` then cross-checks
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Chandlery capability registry engine.")
